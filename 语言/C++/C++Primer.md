...
#### 栈展开
指的是沿着函数调用链一路向上寻找匹配的 `catch` 字句。找不到匹配的 `catch` 字句时则调用标准库函数 `terminate`

#### 栈展开过程中对象被自动销毁
栈展开时，调用链上提前退出的函数可能已经创建了一些局部对象，编译器会负责销毁这些变量，对于内置类型它不需要做任何事情，对于类类型它会自动调用其析构函数。

如果异常发生在类构造到一半的时候，需要正确地销毁已经构造的部分。相似地，如果异常发生在数组或标准库容器初始化了一部分时，也需要正确销毁已经被构造的元素。

#### 析构函数与异常
析构函数总是会被执行，而其普通函数中释放资源的代码可能被跳过，这影响程序的结构。如果在语句块中分配资源并在末尾释放，则如果中间发生异常资源就无法被正确释放；类本身分配的资源需要通过析构函数释放。因此，如果选择使用类来控制资源，比较容易保证资源管理的异常安全。

析构函数会在栈展开的时候被调用，此时如果它抛出异常并未能自行捕获，则系统调用 `terminate` 结束程序。故析构函数往往不应该抛出自己无法处理的异常。如果它的某个操作会抛出异常，应该包裹在 `try` 块中，并在 `catch` 块中处理好。实际的编程过程中，因为析构函数仅仅释放资源，不太会抛出异常。

所有标准库类型确保析构函数不抛出异常。

#### 异常对象
这是一种特殊的对象，编译器使用异常抛出表达式对异常对象进行拷贝初始化。因此， `throw` 语句中的表达需要具有完全类型。如果它是类类型，必须具有一个可访问的析构函数，以及拷贝或移动构造函数。如果是数组或函数类型，它会被转换为指针类型。

如果抛出一个指针，它不应该指向局部的对象，因为当栈展开时该对象会被销毁。

当抛出一条表达式的时候，它的静态类型决定了异常对象的类型。如果一条 `throw` 表达式解引用一个指向派生类对象的基类指针，则派生类部分会被切掉。

### 捕获异常
`catch` 子句声明的类型必须是完全类型，可以是左值引用，但不能是右值引用。如果它不是引用，则会是异常对象的拷贝。如果是引用，则会绑定到异常对象上。

如果 `catch` 参数的类型是基类类型，可以捕获派生类异常对象，如果它是引用，则发生引用的转换；如果它不是引用，那么派生类部分被切掉。

#### 查找匹配的处理代码
`catch` 子句的匹配不是寻找最佳匹配，而是按顺序查找第一个可用的匹配，故派生类的 `catch` 子句应该放在基类的 `catch` 子句之前。相比于函数参数的匹配， `catch` 匹配的规则有更多的限制，基本上 `catch` 匹配要求是精确匹配的，除了以下规则：
- 允许从非常量向常量的转换，即参数为常量引用的 `catch` 子句可以匹配非常量的异常对象
- 允许从派生类向基类的转换
- 数组和函数被转换成指针类型

#### 重新抛出
`catch` 子句无法完全地处理异常的时候，可以重新抛出该异常：
```cpp
throw;
```
该语句只能出现在 `catch` 块内，或者通过 `catch` 块内语句直接或间接调用的函数内。否则，编译器会调用 `terminate` 。
...
```
它可以单独出现，也可以和别的 `catch` 子句一起出现，但是必须是最后一个 `catch` 子句，否则其他的 `catch` 子句永远不会被执行。

### 函数try语句块与构造函数
一般来说，程序的任何时刻都可能发生异常，特别是可能在处理构造函数初始值的过程中，此时无法被构造函数体内的 `try` 块包裹。C++提供了**函数try语句块**（函数测试块）来处理这种情况：
```cpp
template <typename T>
...
```
这里的 `try` 可以同时处理构造函数初始值和构造函数体内的异常。如果构造函数初始值抛出异常，那么构造函数体内的 `catch` 子句会被执行。

如果构造函数的参数构造时抛出异常，这不归这里的 `try` 块管，应该由构造函数调用者来处理。

### noexcept异常说明
C++11提供了 `noexcept` 说明表示某个函数不会抛出异常：
```cpp
void f() noexcept;
```
称 `noexcept` 做了**不抛出说明**。它需要出现在所有声明和定义中，如果函数使用尾置返回类型，则它应该出现在尾置返回类型之前。对于成员函数，它应该出现在 `const` 和引用限定符之后， `final` 、 `override` 或 `=0` 之前。

在函数指针的声明和定义中也可以指定 `noexcept` ，在 `typedef` 和类型别名中则不行。

#### 违反异常说明
编译器实际上并不检查 `noexcept` 说明是否被遵守，故在 `noexcept` 函数内抛出异常也可以通过编译，然而程序会直接调用 `terminate` 。是否会有栈展开的过程没有约定，因此除了在函数不会抛出异常的情况下使用 `noexcept` ，也可以在不止异常如何处理时使用。

#### 向后兼容：异常说明
早期的C++可以指出函数会抛出哪些异常，这种说明被称为**异常说明**。它的形式是 `throw(type-list)` ，其中 `type-list` 是一个以逗号分隔的类型列表。C++11中，异常说明方案被取消，但仍有一个用途：
```cpp
// 两条语句等效
void f() noexcept;
void f() throw();
...
#### 异常说明的实参
`noexcept` 可以接受一个实参，它需要转换成 `bool` 类型：
```cpp
...
#### noexcept运算符
`noexcept` 的实参常常与 `noexcept` 运算符一起使用。它是一个一元运算符，返回一个 `bool` 类型的右值常量表达式，用于表示给定的表达式是否会抛出异常。和 `sizeof` 类似，它也不会对表达式求值。形式如：
```cpp
noexcept(expr);
```
如果 `expr` 调用的所有函数都做了 `noexcept` 说明并且 `expr` 本身不含 `throw` 语句时，上述表达式的值为 `true` ，否则为 `false` 。因此可以这样使用：
```cpp
void f() noexcept(noexcept(g()));   // f和g的异常说明情况一致
```

#### 异常说明与指针、虚函数和拷贝控制
函数指针和该指针所指的函数必须具有一致的异常说明。声明了不抛出异常的函数指针只能指向不抛出异常的函数，否则可以指向所有的函数。如：
```cpp
void (*pf1)(int) noexcept = recoup;
```
如果一个虚函数承诺了不会抛出异常，后续派生的虚函数也必须做同样的承诺。如果虚函数允许抛出异常，派生版本则不受限制。

编译器合成拷贝控制成员的时候，也生成异常说明。如果所有成员和基类的所有操作都不会抛出异常，那么合成的成员是 `noexcpet` 的。如果合成成员调用的某个操作可能抛出异常，则合成的成员是 `noexcept(false)` 的。

如果定义了析构函数并不提供异常说明，则编译器将合成一个，这个异常声明与假设编译器自行合成析构函数生成的异常声明一致。
...
类型 `exception` 只定义了拷贝构造函数、拷贝赋值运算符、一个虚析构函数和一个名为 `what` 的虚成员。其中 `what` 函数返回一个 `const char *`，该指针指向一个以 `null` 结尾的字符数组，并且保证不抛出任何异常。

类 `exception` 、 `bad_cast` 和 `bad_alloc` 定义了默认构造函数。类 `runtime_error` 和 `logic_error` 没有默认构造函数，但是有一个可以接受C风格字符串或 `string` 的构造函数，参数用来提供异常的说明。

因为 `what` 是虚函数，如果按照引用的方式来捕获异常，最后调用的是对象动态类型的版本。

## 命名空间
如果多个库都把名字放在全局命名空间中将发生**命名空间污染**，即使用了相同的名字导致冲突。传统上程序员通过设置非常长的名字来避免这样的问题，但**命名空间**是更好的方案。
...
```
只要能放在全局作用域的声明就可以放入命名空间内。命名空间可以出现在命名空间内，但是不能出现在类内和函数内。

#### 每个命名空间都是一个作用域
定义在命名空间中的名字可以被命名空间中的其他成员直接访问，也可以被内层作用域访问。对于命名空间外，需要通过作用域运算符 `::` 来访问。
...
```
可能是定义一个新的命名空间 `nsp` ，或是为已经存在的命名空间新增一些成员，这使得可以把命名空间的定义分散在多个文件中。可以在头文件中定义命名空间的类，而把类成员的定义放在源文件中。

#### 定义命名空间成员
可以在命名空间的外部定义命名空间内的成员，但是必须是该命名空间的外层空间中，而不能在一个不相关的作用域中定义它。

#### 模板特例化
模板特例化必须定义在模板所在的命名空间中，而该实例可以定义在命名空间外部，如：
```cpp
namespace std {
...
#### 全局命名空间
全局作用域中定义的名字都存在于全局命名空间中：
```cpp
::member_name
```

#### 嵌套的命名空间
外层命名空间要使用内层命名空间的名字，需要使用作用域运算符。

#### 内联命名空间
C++11引入了一种新的嵌套命名空间，称为**内联命名空间**，其中的名字可以直接被外层命名空间使用：
```cpp
inline namespace FifthEd {      // 第一次定义必须写inline
    // ...
}

namespace FifthEd {             // 后续可以不写，隐式内联
    // ...
}
```
常常在版本迭代中使用，去除老版本的 `inline` 再给新版本加上 `inline` ，这样使得访问到的名字都来自新版本。而旧版本的命名空间也可以留在代码中，但是需要用作用域运算符来访问。
...
```
其中的变量具有静态生命周期，在第一次使用前被创建，程序结束才被销毁。它可以在一个文件内不连续，每个文件的未命名命名空间都是独立的。未命名的命名空间中定义的名字的作用域，与命名空间自身所在的作用域相同。全局作用域中的未命名命名空间里的变量如同全局变量一样可以直接使用，因此需要与全局变量区分开。

未命名的命名空间也可以嵌套定义在其他命名空间中，此时需要通过作用域运算符来访问其中的名字：
```cpp
namespace local {
    namespace {
        int i;
...
#### using声明：扼要概述
一条 `using` 声明一次引入命名空间的一个成员，它的有效范围从声明开始，直到所在的作用域结束。它可以出现在全局作用域、局部作用域、命名空间作用域和类作用域中，在类中它只能指向基类成员。

#### using指示
一次性引入命名空间中的所有名字，直到作用域结束都能使用。

#### using指示与作用域
`using` 指示并不只是像 `using` 声明一样简单创建个别名一样。因为有的名字可能不好在内层作用域出现，往往编译器寻找指示的命名空间与当前作用域的公共外层作用域，并把指示的命名空间中的所有名字注入公共外层作用域中，如：
```cpp
namespace A {
    int i, j;
}
...
void f()
{
    using namespace A;  // 把A中的所有名字注入全局作用域
    i = 5;              // 二义性错误
}
```
...
std::cin >> s;
```
这里用到的 `operator>>` 定义在 `std` 命名空间中，却可以直接使用。这是因为名字查找的一个例外规则：给函数传递一个类类型的实参时，除了常规的作用域查找以外还会查找类所在的命名空间。对于传递类的引用和指针也同样有效。

#### 查找与std::move和std::forward
通常情况下，如果应用程序使用的名字和标准库的冲突，那么会发生两种情况之一：
- 编译器根据一般重载规则选择其中一个函数使用
- 编译器不使用标准库的函数

由于 `move` 和 `forward` 冲突的可能比较大，这两个函数建议直接使用 `std::` 的调用方法。

#### 友元声明与实参相关的查找
一个未声明的类或函数如果第一次出现在友元声明中，它会在外层的命名空间被隐式声明，这可能带来一些奇怪的现象，如：
```cpp
namespace A {
    class C {
...
#### 与实参相关的查找与重载
由于函数接受类类型实参会改变查找规则，每个实参类所属命名空间的名字都会加入候选集，即使某些函数在调用处不可见。

#### 重载与using声明
`using` 声明的是一个名字，而不是一个函数：
```cpp
using NS::print(int);       // 错误：不能指定形参列表
using NS::print;            // 正确：引入了所有的print
```
`using` 引入的函数会隐藏外层作用域中的同名函数，如果当前作用域有同名且形参相同的函数则会报错，如果形参不同则构成重载。

#### 重载与using指示
`using` 指示引入的函数如果发生同名同形参的情况，并不会报错，但是需要指明调用哪个函数。

## 多重继承与虚继承
多重继承指从多个直接基类中产生派生类的能力。

### 多重继承
继承列表只能包含已经定义过的类，这些类不能是 `final` 的，而且每个类只能出现一次。

#### 派生类构造函数初始化所有基类
基类的构造顺序与派生类列表中基类的出现顺序保持一致，与初始值列表中出现顺序无关。

#### 继承的构造函数与多重继承
C++11可以用 `using` 继承基类的构造函数，在多重继承中也可以从多个直接基类继承构造函数。如果继承了两个具有同样形参的构造函数，会报错，此时派生类必须自己定义一个版本。

#### 析构函数与多重继承
派生类的析构函数只负责清除自己的资源，派生类的成员和基类都是自动销毁的，合成的析构函数体为空。基类析构函数的调用顺序与构造函数的调用顺序相反。

#### 多重继承的派生类的拷贝与移动操作
如果派生类定义了自己的拷贝/赋值构造函数和拷贝运算符，必须在完整的对象上执行拷贝、移动或赋值操作。只有使用合成版本的时候，会对基类部分自动进行这些操作，调用基类操作的顺序与构造函数的调用顺序相同。

### 类型转换与多个基类
派生类的指针可以转换成任意基类的指针，在编译器看来这些转换都一样好。

### 多重继承下的类作用域
在多重继承下，名字查找的过程同时顺着所有直接基类进行，如果一个名字在两颗继承树中出现，那么将引发二义性错误，这时需要明确指出需要的是哪个版本。

比较复杂的是，编译器会先进行名字查找再进行类型检查。因此，即使从两个基类继承来同名的不同参数成员也会出错；即使继承来一个公有成员和一个私有成员也会出错。

为了避免二义性问题，可以自行定义一个同名的成员，这样就隐藏了基类的同名成员。

### 虚继承
虚继承表示一个类做出声明，承诺愿意共享它的基类。被共享的基类对象称为**虚基类**。
...
```
这里的 `public` 和 `virtual` 的顺序无关紧要。

#### 支持向基类的常规类型转换
派生类的指针也可以转换成虚基类的指针。

#### 虚基类成员的可见性
如果虚基类有成员 `x` ，两个类分别继承了虚基类，另外一个类继承了这两个类。如果两个中间类都没有覆盖 `x` ，那么在派生类中 `x` 指的是虚基类的 `x` ；如果只有一个覆盖了 `x` ，因为这个 `x` 的优先级比虚基类的 `x` 高，所以在派生类中 `x` 指的是覆盖的 `x` ；如果两个中间类都覆盖了 `x` ，那么在派生类中 `x` 具有二义性。

### 构造函数与虚继承
虚基类是由最低层的派生类初始化的，而继承体系中的每个类都有可能成为最低层的派生类。因此，每个派生类的构造函数都要初始化虚基类，即使虚基类不是它的直接基类，它的构造函数也可以调用虚基类的构造函数。
