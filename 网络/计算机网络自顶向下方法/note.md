...
![平均排队时延与流量强度](image/平均排队时延与流量强度.png)
...
![rdt301](image/rdt301.png)
![rdt302](image/rdt302.png)
...
![TCP客户端状态](image/TCP客户端状态.png)

服务端会经历的状态如图：

![TCP服务端状态](image/TCP服务端状态.png)
...
![有限缓存路由器和多跳路径的性能](image/有限缓存路由器和多跳路径的性能.png)
...
#### 优先权排队
![优先权排队](image/优先权排队.png)

可以有多个具有不同优先级的队列，算法优先传输高优先级队列中的分组，队列中的分组选择常常使用FIFO。

#### 循环和加权公平排队
![循环和加权公平排队](image/循环和加权公平排队.png)
...
![链路状态振荡1](image/链路状态振荡1.png)

![链路状态振荡2](image/链路状态振荡2.png)

![链路状态振荡3](image/链路状态振荡3.png)

![链路状态振荡4](image/链路状态振荡4.png)
...
### 数据平面和控制平面交互的例子
![数据平面和控制平面交互](image/数据平面和控制平面交互.png)
...
![VLAN扩展以太网帧](image/VLAN扩展以太网帧.png)
...
### 无线主机
主机是运行应用程序的端系统设备，本身可能移动也可能不移动

### 无线链路
主机通过**无线通信链路**连接到基站和另一台无线主机，不同的无线链路技术提供不同的传输速率和覆盖范围，如下图
    ![无线网络标准](image/无线网络标准.png)

### 基站
**基站**是无线网络基础设施的一个关键部分，不同于上述两个要素，它没有在有线网络体系中的对应物。它负责和无线主机交换数据，并且需要协调多个无线主机的传输。常见的基站有蜂窝网络的**蜂窝塔**和802.11无线LAN中的**接入点**。

当说某台主机和基站相关联，指的是它位于基站的覆盖范围内，并且主机使用基站作为中继和更大的网络交互。与基站关联的主机通常被称为以**基础设施模式**运行，此时所有传统网络服务（如地址分配和路由选择等）都由基站提供；在**自组织网络**中，无线主机没有这样的基础设施，主机自身需要提供这些服务。

当一台主机从一个基站的覆盖范围移动到另一个基站的覆盖范围，它会改变关联的基站，这个过程称为**切换**。这会带来许多的问题，如TCP连接可能会中断等等。

### 网络基础设施
无线主机希望能与之通信的更大网络。实际上无线网络体系可以通过两个指标来分类：
- 无线网络中的分组跨越一个无线跳还是多个无线跳
- 网络中是否有基站这样的基础设施

因此可以有以下四类无线网络：
- 单跳，基于基础设施：具有一个基站，它与所有主机的通信都经过单个无线跳。802.11无线LAN和4G LTE都属于这一类
- 单跳，无基础设施：没有基站，但是一个节点可以协调其他节点的传输。蓝牙网络和具有自组织模式的802.11网络属于这一类
- 多跳，基于基础设施：有基站，但是某些主机为了到达基站，可能需要通过其他无线节点进行中继。某些无线传感网络和**无线网状网络**属于这一类
- 多跳，无基础设施：没有基站，节点为了到达目的地可能需要经过多个无线节点中继。节点可以是移动的，在多个节点中改变连接关系，它被称为**移动自组织网络**。如果这样的移动节点是车载的，那么这样的网络被称为**车载自组织网络**。

## 无线链路和网络特征
考虑使用无线802.11网络替换一个有线以太网，其中无线网络接口代替主机的有线以太网接口，接入点代替以太网交换机，而其他部分则不需要大的改变，也就是说有线网络和无线网络主要的区别在于链路层。实际上有如下的区别：
- 递减的信号强度：电磁波穿过障碍物强度会减弱。即使是自由空间中也会随着距离的增加而减弱（称为**路径损耗**）
- 来自其他源的干扰：在同一个频段发送的信号会相互干扰，如2.4GHz无线电话和802.11b无线LAN在相同的频段传输。另外，电磁噪声也会干扰。
- 多径传播：电磁波的一部分可能在墙面反射，从而在发送方和接收方之间出现路径长度不同的多个信号，影响接收方对于信号的接收，这称为**多径传播**。

由于这些原因，比特差错在无线网中更加普遍，因此无线网使用CRC校验并且使用链路层的ARQ协议重传受损的帧。

当主机收到一个电磁信号，其中混合了原始信号的退化形式和其他的噪声，用**信噪比SNR**来衡量信号和噪声强度的相对大小，单位是dB。显然SNR越大，接收方更容易从其中还原出原始信号。

令**比特差错率BER**为接收方接收的某个比特有错的概率，则下图表现出了三种调制技术中BER和SNR的关系：

![BER和SNR](image/BER和SNR.png)

根据图片可以看出：
- 同一个调制方案下，BER随着SNR的增加而减小。发送方可以通过增大功率来提高SNR，然而增大到一定程度以后就收效甚微，再增大功率只是浪费能量
- 同样的SNR下，使用较高比特速率的调制方案BER更高
- 物理层调制技术的动态选择能够自适应各种信号条件

相比于有线网络，无线网络除了比特差错率更高，还会有**隐藏终端问题**：当A向B发送数据并且C也在向B发送数据，此时在B处发生了信号干扰，然而可能由于A和C之间存在障碍物的遮挡使得它们无法检测到对方正在传输数据。或是A和C的距离太远，**衰减**使得对方的信号传输到自己处时已经显得微不足道。

![隐藏终端问题和衰减](image/隐藏终端问题和衰减.png)

### CDMA
在有线网络中介绍了三种媒体访问协议：信道划分、随机访问和轮流。码分多址CDMA从属于信道划分协议族，主要用于无线LAN和蜂窝技术。

在CDMA中，每个要发送的比特都乘以一个信号（编码）来进行编码，该信号的变化速率（**码片速率**）比数据序列变化速率快得多。一个简单的、理想化的CDMA编解码情形如下：

![CDMA编解码](image/CDMA编解码.png)

假设每个比特发送需要一个时隙，图中 $d_i$ 为第 $i$ 个时隙中要发送的数据比特，为了数学推导的方便，数据取值0时令 $d_i=-1$。每个时隙进一步划分成 $M$ 个微时隙，在图中 $M=8$ ，实际情况 $M$ 往往比这大很多。发送方使用一个 $M$ 比特的序列 $c$ 来编码数据。

显然输出的编码
$$
Z_{i, m} = d_i \cdot c_m
$$
加入信号没有衰减和干扰，那么对方可以完整地收到 $Z_{i, m}$ ，然后如下还原出数据：
$$
d_i = \frac{1}{M}\sum_{m=1}^M Z_{i, m} \cdot c_m
$$
但这样的假设是不现实的，实际上信号会受到干扰和衰减。当两个发送方发送的数据混在一起时，接收方假设信号强度是加性的，即两个发送方发送信号1则接收方接收到的信号是2，两个发送方分别发送1和-1则接收方接收到的信号是0。设发送方 $s$ 发出的信号为 $Z_{i, m}^{*}$ 则接收方接收到的信号为
$$
Z_{i, m}^{*} = \sum_{s = 1} Z_{i, m}^s
$$
如果多个发送方选择的编码序列 $c$ 两两正交，则接收方可以通过如下公式还原出数据：
$$
d_i = \frac{1}{M}\sum_{m=1}^M Z_{i, m}^{*} \cdot c_m
$$

CDMA之所以是一个划分协议，在于它划分了编码空间。实际使用中需要考虑许多问题，如何选择编码序列；接收方接收到来自不同发送方的信号强度不同等等。

## WiFi：802.11无线LAN
曾经出现过多种无线LAN的标准和技术，然而**IEEE 802.11无线LAN**（也称**WiFi**）标准已经成为了主流。其中有几套不同的标准，都使用CSMA/CA媒体访问协议，并且使用相同的链路层帧结构，都具有降低速率延伸到更远距离的能力。另外802.11的产品是可以后向兼容的，即老旧的802.11g设备可以和802.11ac基站通信。它们在下表中列出：

![802.11标准](image/802.11标准.png)

它们在物理层上有一些重要的区别。2.4GHz频段无需执照，但大量的设备可能会争用该频段。5GHz在同功率下传播距离更短，并更容易受到多径传播的影响。802.11n和802.11ac使用了**多输入多输出MIMO**天线，发送方的多条天线可以发送不同的信号，接收方的多条天线可以接收不同的信号。802.11ac基站可以同时向多个站点传输，并使用智能天线在接收方的方向上使用自适应成型波束来传输，减少了干扰并增大了传输距离。

### 802.11体系结构
![802.11体系结构](image/802.11体系结构.png)

上图显示了802.11网络体系结构。其中的基本构件模块是**基本服务集BSS**，一个BSS包含一个或多个无线站点，以及一个称为**接入点AP**的中央**基站**。图中的两个AP连接到一个互联设备，可以是交换机或者路由器，然后该设备连接到因特网。